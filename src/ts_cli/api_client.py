...
class APIClient:
    ...
    async def send_analysis(
        ...
        try:
            ...
            self.logger.info(f"분석 데이터 전송 완료. 분석 ID: {response_data.get('analysis_id')}")
    ...
    async def download_result(
        ...
        try:
            ...
            if download_path is None:
                download_path = Path.cwd() / f"testscenario_result_{self._get_timestamp()}.zip"
    ...
    async def _handle_response(self, response: httpx.Response) -> None:
        ...
        try:
            ...
            error_message = error_data.get('message', f'HTTP {response.status_code} 오류')
