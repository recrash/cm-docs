...
from .vcs.base_analyzer import RepositoryError, UnsupportedVCSError, InvalidRepositoryError
...
class CLIHandler:
    ...
    def analyze_repository(self, path: Path) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[red]예상치 못한 오류가 발생했습니다: {str(e)}[/red]"
            )
            if self.verbose:
        ...
        try:
            ...
            if self.verbose:
                ...
                if repo_info.get('current_branch'):
                    self.console.print(f"  현재 브랜치: [cyan]{repo_info['current_branch']}[/cyan]")
            
            return analyzer
            
        except Exception as e:
            self.logger.error(f"저장소 검증 중 오류: {e}")
            self.console.print(f"[red]저장소 검증 중 오류가 발생했습니다: {str(e)}[/red]")
            return None
    
    def _analyze_changes(self, analyzer) -> Optional[Dict[str, Any]]:
        """
        저장소 변경사항 분석
        ...
        try:
            ...
            self.console.print(f"[red]변경사항 분석 중 오류가 발생했습니다: {str(e)}[/red]")
            return None
        ...
        try:
            ...
            if self.verbose:
                ...
                if response.get('analysis_id'):
                    self.console.print(f"  분석 ID: [cyan]{response['analysis_id']}[/cyan]")
