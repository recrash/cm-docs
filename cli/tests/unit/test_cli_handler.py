...
class TestCLIHandler:
    ...
    def test_validate_repository_success(
        self, mock_get_analyzer, cli_handler, mock_analyzer, tmp_path
    ):
        """저장소 유효성 검사 성공 테스트"""
    ...
    @patch("ts_cli.cli_handler.get_analyzer")
    def test_validate_repository_not_supported(
        self, mock_get_analyzer, cli_handler, tmp_path
    ):
        """지원되지 않는 저장소 타입 테스트"""
    ...
    def test_analyze_changes_success(self, cli_handler, mock_analyzer):
        ...
        assert (
            result["repository_info"] == mock_analyzer.get_repository_info.return_value
        )
    ...
    def test_display_final_result_json_format(self, cli_handler_json):
        ...
        api_response = {"analysis_id": "test-123", "status": "completed"}
    ...
    def test_analyze_repository_validation_failure(
        self, mock_validate, cli_handler, tmp_path
    ):
        """저장소 검증 실패 테스트"""
