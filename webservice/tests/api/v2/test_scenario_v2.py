...
class TestV2ScenarioAPI:
    ...
    def test_generate_endpoint_success(self, client, sample_request):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            response = client.post("/api/webservice/v2/scenario/generate", json=sample_request)
    ...
    def test_generate_endpoint_invalid_repo_path(self, client):
        ...
        with patch('app.api.routers.v2.scenario_v2.Path') as mock_path_class:
            ...
            response = client.post("/api/webservice/v2/scenario/generate", json=invalid_request)
    ...
    def test_generate_endpoint_duplicate_client_id(self, client, sample_request):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            # 첫 번째 요청
            response1 = client.post("/api/webservice/v2/scenario/generate", json=sample_request)
            assert response1.status_code == 200
            
            # 동일한 클라이언트 ID로 두 번째 요청
            response2 = client.post("/api/webservice/v2/scenario/generate", json=sample_request)
    ...
    def test_generate_endpoint_missing_fields(self, client):
        ...
        response = client.post("/api/webservice/v2/scenario/generate", json=incomplete_request)
        assert response.status_code == 422  # Validation error
    
    def test_generate_endpoint_invalid_json(self, client):
        """잘못된 JSON 형식 테스트"""
        response = client.post(
            "/api/webservice/v2/scenario/generate", 
            data="invalid json",
    ...
    def test_status_endpoint(self, client):
        ...
        response = client.get(f"/api/webservice/v2/scenario/status/{client_id}")
        assert response.status_code == 200
    ...
    def test_status_endpoint_active_generation(self, client, sample_request):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            # 상태 조회
            status_response = client.get(f"/api/webservice/v2/scenario/status/{sample_request['client_id']}")
