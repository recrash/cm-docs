...
def test_get_scenario_config(client):
    """시나리오 설정 조회 테스트"""
    response = client.get("/api/webservice/scenario/config")
...
def test_generate_scenario_success(client, mock_dependencies):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('app.core.git_analyzer.get_git_analysis_text', return_value="Mock Git analysis for testing"), \
            ...
            # WebSocket 연결 테스트
            with client.websocket_connect("/api/webservice/scenario/generate-ws") as websocket:
                request_data = {
                    "repo_path": "/test/repo",
...
def test_generate_scenario_from_text_success(client, mock_dependencies):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('app.api.routers.scenario.load_config', return_value=config_data), \
            ...
            response = client.post("/api/webservice/scenario/v1/generate-from-text", json=request_data)
