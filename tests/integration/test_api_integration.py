...
class MockServer:
    ...
    def add_response(self, method: str, path: str, response_data: dict, status_code: int = 200):
        """응답 설정"""
...
class TestAPIIntegration:
    ...
    async def test_send_analysis_full_workflow(self, api_client, mock_server):
        ...
        with patch.object(api_client.client, 'post') as mock_post:
            ...
            mock_response.json.return_value = mock_server.responses['POST:/api/v1/analysis']['data']
            ...
            # 요청이 올바르게 호출되었는지 확인
            mock_post.assert_called_once_with('/api/v1/analysis', json={
...
class TestFullWorkflowIntegration:
    ...
    @patch('ts_cli.vcs.git_analyzer.subprocess.run')
    def test_full_analysis_workflow_dry_run(
        self, 
        mock_subprocess,
        mock_git_repo
    ):
    ...
    def test_configuration_integration(self):
        """설정 통합 테스트"""
        from ts_cli.utils.config_loader import load_config, get_api_config, get_cli_config
...
class TestErrorScenarios:
    ...
    def test_git_command_failure(self, mock_subprocess, tmp_path):
        ...
        mock_subprocess.side_effect = [
            ...
            Mock(returncode=1, stdout="", stderr="fatal: not a git repository")  # get_changes
        ]
