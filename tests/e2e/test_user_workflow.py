...
class TestCLIUserWorkflow:
    ...
    def temp_git_repo(self):
        ...
        try:
            # Git 저장소 초기화
            subprocess.run(
                ["git", "init"], cwd=str(temp_dir), check=True, capture_output=True
            )
            subprocess.run(
            ...
            subprocess.run(
                ["git", "commit", "-m", "Initial commit"], cwd=str(temp_dir), check=True
            )

            # 변경사항 추가
            test_file.write_text(
                'print("Hello, TestscenarioMaker!")\nprint("New feature added")'
            )
    ...
    def test_cli_help_command(self, cli_executable):
        ...
        result = subprocess.run(
            cli_executable + ["--help"], capture_output=True, text=True, timeout=30
        )
    ...
    def test_cli_version_command(self, cli_executable):
        """버전 명령어 테스트"""
        result = subprocess.run(
            cli_executable + ["version"], capture_output=True, text=True, timeout=30
        )
    ...
    def test_cli_invalid_repository(self, cli_executable, tmp_path):
        ...
        # stdout에서 메시지 확인 (Rich console이 stdout에 출력)
        assert "지원되지 않는" in result.stdout or "유효하지 않은" in result.stdout
    ...
    def test_cli_verbose_mode(self, cli_executable, temp_git_repo):
        ...
        result = subprocess.run(
            cli_executable
            + ["analyze", "--path", str(temp_git_repo), "--verbose", "--dry-run"],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_cli_config_show_command(self, cli_executable):
        """설정 표시 명령어 테스트"""
        result = subprocess.run(
            cli_executable + ["config-show"], capture_output=True, text=True, timeout=30
        )
    ...
    def test_cli_with_mock_api_success(
        self, mock_client_class, cli_executable, temp_git_repo
    ):
        ...
        result = subprocess.run(
            cli_executable + ["analyze", "--path", str(temp_git_repo), "--dry-run"],
            capture_output=True,
            text=True,
            timeout=60,
        )
...
class TestCLIPerformance:
    ...
    def large_git_repo(self):
        ...
        try:
            ...
            subprocess.run(["git", "add", "."], cwd=str(temp_dir), check=True)
            subprocess.run(
                ["git", "commit", "-m", "Large commit"], cwd=str(temp_dir), check=True
            )
    ...
    def test_cli_performance_large_repo(self, large_git_repo):
        ...
        result = subprocess.run(
            cli_executable + ["analyze", "--path", str(large_git_repo), "--dry-run"],
            capture_output=True,
            text=True,
            timeout=120,  # 2분 타임아웃
        )
...
"""
        ...
        result = subprocess.run(
            cli_executable + ["config-show", "--config", str(config_file)],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
class TestCLIOutputValidation:
    ...
    def test_cli_json_output_format(self, tmp_path):
        ...
        result = subprocess.run(
            cli_executable
            + ["analyze", "--path", str(tmp_path), "--output", "json", "--dry-run"],
            capture_output=True,
            text=True,
            timeout=60,
        )
    ...
    def test_cli_output_file_creation(self, tmp_path):
        ...
        result = subprocess.run(
            cli_executable + ["analyze", "--path", str(tmp_path), "--dry-run"],
            capture_output=True,
            text=True,
            timeout=60,
        )
